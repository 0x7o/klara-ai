...
class WeatherQuery:
    ...
    def get_weather_tts(self, data, date, place_name):
        ...
        if date == "today":
            ...
            tts += f"Завтра в {place_name} {self.convert_temp_to_words(data['main']['temp'])} градусов, {data['weather'][0]['description']}, ощущается как {self.convert_temp_to_words(data['main']['feels_like'])}."
        elif date == "after_tomorrow":
            tts += f"Послезавтра в {place_name} {self.convert_temp_to_words(data['main']['temp'])} градусов, {data['weather'][0]['description']}, ощущается как {self.convert_temp_to_words(data['main']['feels_like'])}."
        else:
